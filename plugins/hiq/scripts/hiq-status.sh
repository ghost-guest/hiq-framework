...
hook_last_run="$(normalize_none "$hook_last_run")"
hook_last_event="$(normalize_none "$hook_last_event")"
...
[[ -n "$host_evidence" ]] || host_evidence="$(md_field host_automation_evidence "$SESSION")"
[[ -n "$hook_protocol" ]] || hook_protocol="$(md_field hook_protocol_version "$SESSION")"
[[ -n "$hook_core" ]] || hook_core="$(md_field hook_core_status "$SESSION")"
[[ -n "$hook_adapter" ]] || hook_adapter="$(md_field hook_adapter "$SESSION")"
[[ -n "$hook_last_event" ]] || hook_last_event="$(md_field hook_last_event "$SESSION")"
[[ -n "$hook_last_run" ]] || hook_last_run="$(md_field hook_last_run "$SESSION")"
[[ -n "$hook_last_status" ]] || hook_last_status="$(md_field hook_last_status "$SESSION")"
...
if $JSON_MODE; then
  ...
  printf '  "hostAutomationEvidence": "%s",\n' "$(json_escape "$host_evidence")"
  printf '  "hookProtocolVersion": "%s",\n' "$(json_escape "$hook_protocol")"
  printf '  "hookCoreStatus": "%s",\n' "$(json_escape "$hook_core")"
  printf '  "hookAdapter": "%s",\n' "$(json_escape "$hook_adapter")"
  printf '  "hookLastEvent": "%s",\n' "$(json_escape "$hook_last_event")"
  printf '  "hookLastRunPath": "%s",\n' "$(json_escape "$hook_last_run")"
  printf '  "hookLastRunStatus": "%s",\n' "$(json_escape "$hook_last_status")"
...
echo "host_automation_evidence=${host_evidence:-none}"
echo "hook_protocol_version=${hook_protocol:-unknown}"
echo "hook_core_status=${hook_core:-unknown}"
echo "hook_adapter=${hook_adapter:-none}"
echo "hook_last_event=${hook_last_event:-none}"
echo "hook_last_run=${hook_last_run:-none}"
echo "hook_last_status=${hook_last_status:-none}"
